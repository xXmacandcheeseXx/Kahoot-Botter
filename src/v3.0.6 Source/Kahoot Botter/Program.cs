...
namespace Kahoot_Botter
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.Title = "Kahoot Botter | v3.0.6";
        ...
        F:
            ...
            else if (rs.ToLower() == "s")
            {
                ...
                Console.Clear();
                Typecbp();
                Console.WriteLine("Type or from file? (For special characters) | (T/F)");

                string tf = Console.ReadLine();

                if (tf.ToLower() != "t" && tf.ToLower() != "f")
                {
                    goto F;
                }
                ...
                else if (tf.ToLower() == "f")
                {
                    Console.Clear();
                FROMFILE:
                    Typecbp();
                    Console.WriteLine("NOTE: If there are special characters in the name they will most likely show up as \"?\" in the console\nBUT your name will still come out as expected in kahoot\n");
                    Console.WriteLine("Put your name of choice inside the newly created \"name.txt\" file");
                    ...
                    try
                    {
                        prtest = File.ReadAllText(Directory.GetCurrentDirectory() + "\\name.txt");
                    }
                    catch (FileNotFoundException)
                    {
                        Console.Clear();
                        Console.WriteLine("File was moved or deleted | Try again");
                        Console.ReadKey();
                        Console.Clear();
                        goto FROMFILE;
                    }
        ...
        static void StartBotting()
        {
            ...
            for (int i = 0; i < Browser.BotsToSend; i++)
            {
                // --------------------------------------
                browser.InitTabs();
                // --------------------------------------
        ...
        {
            string GameCode_temp;
        GAMECODE1:
            Console.Clear();
            Typecbp();

            Console.WriteLine("Type the game code (no spaces)");
            GameCode_temp = Console.ReadLine();

            try
            {
                Convert.ToInt32(GameCode_temp);

                if (GameCode_temp.Length < 4 || GameCode_temp.Length > 8)
                {
                    Console.Clear();
                    Typecbp();
                    Console.WriteLine("Try again");
        ...
        GAMECODE2:
            Console.Clear();
            Typecbp();
            Console.WriteLine("Are you sure \"" + GameCode_temp + "\" is the game code? | (Y/N)");
