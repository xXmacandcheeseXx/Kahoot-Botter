            ...
            else if (SpinForName == false) // IF GAME DOESN'T HAVE FRIENDLY NICKNAMES ON
            {
                ...
                A:
                    ...
                    if (PrefixValue.Length > 11)
                    {
                        ...
                        ((IJavaScriptExecutor)driver).ExecuteScript("arguments[0].setAttribute('maxlength','');", element); // remove character limit from nickname box if name is greater than 11 characters (because if it is lower there is no need)
                    }
